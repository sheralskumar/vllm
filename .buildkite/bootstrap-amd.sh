#!/bin/bash
# Fork-local AMD CI bootstrap: uses .buildkite/test-template-amd.j2 from this repo
# instead of curling vllm-project/ci-infra (see pipeline-amd.yaml entry point).
...
upload_pipeline() {
    ...
    if [[ ! -f .buildkite/test-template-amd.j2 ]]; then
        buildkite-agent annotate --style error 'Missing .buildkite/test-template-amd.j2 in repo checkout'
        exit 1
    fi
    cp .buildkite/test-template-amd.j2 .buildkite/test-template.j2
    echo "Using fork-local test-template-amd.j2 (VLLM_CI_BRANCH=${VLLM_CI_BRANCH})"
